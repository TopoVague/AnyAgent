...
public class VariableJPanelCreator {
	// Return a JPanel that contains the GUI component relating to the given
	// variables
	public static JPanel getPanel(String variableName, guiVariableTypes gvt,
			HashMap<String, NameToVariableClass> variableSet) {
		...
		if (gvt == guiVariableTypes.POSITIVE_INT || gvt == guiVariableTypes.DOUBLE) {
			...
			NameToVariableClass assocNvc = variableSet.get(variableName);
			...
			// AgentPanel12_GUI_Alan.addMessage("[" + variableName + "] set to "
			// + text);
			// return true;
			// } else {
			// variableSet.remove(variableName);
			// AgentPanel12_GUI_Alan
			// .addMessage("[" + variableName + "]'s value '" + text + "' is not
			// a " + gvt.toString());
			// return false;
			// }
			// }
			...
			assocNvc.setAssocJtf(jtf); // !! these are using a different type of
										// text field, so the associated field
										// needs to be swapped
		...
		if (gvt == guiVariableTypes.ZERO_TO_ONE_SLIDER) {
			...
			JSlider jsVal = new JSlider(0, 100); // Since there are apparently
													// no decmial types
			...
			variableSet.get(variableName).valueString = jtf.getText(); // setting
																		// default
																		// value
	...
	public static JButton getFileChooserButton(String keyName, guiVariableTypes gvt, JTextField jtf,
			HashMap<String, NameToVariableClass> variableSet) {
		...
		jb.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				chooser.setAcceptAllFileFilterUsed(false);// disable the "All
															// files" option.
	...
	public static void addSetBorderLayoutTitle(Container parent, String title, Boolean capitalized) {
		// Set the parent as border layout, then adds a JTextField to the North
		// with the title capitalized and the text area non editable
	...
	public static Container addGBCComponent(Container parent, Container child, int x, int y, double weightX,
			double weightY) {
